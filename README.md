...
| `confirm-polls` | Consecutive green polls that must observe the same set of jobs before Merge Gatekeeper reports success. GitHub's listings are eventually consistent, so a single green poll can predate CI that has not appeared in the API yet; confirming across polls closes that window. On a merge-queue ref, if the batch merges (deleting the ref) right after a green poll, success is reported — the merge preempted the confirmation. Default is set to 3. |          |
| `empty-grace` | When no jobs are discovered at all, keep polling for this many seconds before reporting success — right after a trigger, CI may simply not have materialized in the API yet. Default is set to 30 (sec).                                                                                            |          |
