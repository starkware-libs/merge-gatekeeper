...
| `confirm-polls` | Consecutive green polls that must observe the same set of jobs before Merge Gatekeeper reports success. GitHub's listings are eventually consistent, so a single green poll can predate CI that has not appeared in the API yet; confirming across polls closes that window. Default is set to 3. |          |
